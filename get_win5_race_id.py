...
import logging
logger = logging.getLogger('get_win5_race_id') #ファイルの名前を渡す
...
def get_win5_racce_id_year(year):
    save_win5_race_id_csv = CSV_DATA_DIR + "/data" + "/win5-" + str(year) + ".csv"
    with open(RACR_URL_DIR+"/"+str(year)+"-win5"+".txt", "r") as f:
        win5_race_id = pd.DataFrame(columns = ["win5_race_id"])
        ...
        win5_race_id = pd.DataFrame(url_list, columns = ["win5_race_id"])
