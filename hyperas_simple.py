...
def hyperas_model(X_train, Y_train, X_test, Y_test):
    ...
    if {{choice(['four','five', 'six'])}} == 'four':
        pass
    elif {{choice(['four','five', 'six'])}} == 'five':
        model.add(Dense({{choice([256,512,1028])}}, kernel_regularizer = regularizers.l2(0.001), activation = 'relu'))
        model.add(Dropout({{uniform(0,1)}}))
    ...
    model.add(Dense(19, activation = 'softmax'))      # output is a number of order

    model.compile(loss = keras.losses.categorical_crossentropy, metrics = ['accuracy'])

    history = model.fit(X_train, Y_train, batch_size = {{choice([64,128,256,512])}}, epochs = {{choice([50,70,100])}}, validation_split = 0.2)
...
def hyperas_simple():
    ...
    best_model.save("model/{}_simple_model.h5".format(OWN_FILE_NAME))
